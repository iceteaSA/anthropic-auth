...
| Commands | `/claude-cache`, `/claude-fast`, `/claude-quota`, `/claude-dump`, `/killswitch` | `/claude-cache`, `/claude-fast`, `/claude-quota`, `/claude-dump` |
...
- The quota toast also shows killed/active status per account.

Manage the killswitch from inside OpenCode:

```text
/killswitch              — show status and command cheatsheet
...
/killswitch set main:3,8 work-alt:5,10 — per-account thresholds
```

Changes made with `/killswitch` are persisted to the sidecar config.
