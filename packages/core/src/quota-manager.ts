...
/**
 * Read the optional refreshEveryNRequests config value.
 * Returns 0 (disabled) if not configured or invalid.
 */
function getQuotaRefreshEveryNRequests(storage: AccountStorage | null): number {
  const n = (storage?.quota as Record<string, unknown> | undefined)
