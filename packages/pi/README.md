...
Priming is off by default. When enabled from OpenCode with `/claude-prime on`, it watches each OAuth account's 5-hour quota reset and sends one real `claude-haiku-4-5` request about one minute after a confirmed reset. This starts the next window without waiting for a normal prompt. Each request uses about 20 input tokens and 1 output token, approximately `$0.000025` at the pricing recorded by this package.
