...
const CORE_PACKAGE_ROOT = resolve(PROJECT_ROOT, 'packages', 'core')
...
// 1. Build first
console.log('[dev] Running initial core build...')
const coreBuild = Bun.spawnSync(['tsc', '-p', 'tsconfig.build.json'], {
  cwd: CORE_PACKAGE_ROOT,
  stdout: 'inherit',
  stderr: 'inherit',
})
...
// 3. Start tsc --watch
console.log('[dev] Starting tsc --watch for core and OpenCode...')
console.log('[dev] Restart OpenCode to pick up the linked plugin.')
const coreChild = Bun.spawn(
...
await Promise.race([coreChild.exited, opencodeChild.exited])
cleanup()
