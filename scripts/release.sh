...
# release.sh — Tag and push a new CortexKit Anthropic auth monorepo release
#
# Usage:
#   ./scripts/release.sh 1.8.0        # release v1.8.0
#   ./scripts/release.sh 1.8.0 --dry  # preview without committing/pushing
...
if [[ -z "$VERSION" ]]; then
  ...
  echo "  e.g. ./scripts/release.sh 1.8.0"
  exit 1
fi
...
echo "  → Watch: https://github.com/cortexkit/anthropic-auth/actions"
